...
def make_exe():
    ...
    # python_config.run_module = "import app; app.main()"

    # Run a Python file when the interpreter starts.
    # python_config.run_filename = ""
    ...
    # attributes.
    # for resource in exe.pip_install(["requests", "paramiko","colorama"]):
...
def make_install(exe):
    ...
    folders = glob([
            "c:/dev/ruspy/oxi/app.py", 
        ], 
        strip_prefix="c:/dev/ruspy/oxi/"
    )
    files.add_manifest(folders)
